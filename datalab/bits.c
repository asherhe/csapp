...
{
  // Uses the two-ANDs an an OR wiring
  // OR created using De Morgan's Laws
  return ~(~(~x & y) & ~(x & ~y));
}
/*
...
int tmin(void)
{
  // tmin is just a 1 followed by all zeros; so we shift 1 by 31 bits
  return 1 << 31;
}
// 2
/*
...
int isTmax(int x)
{
  // tmax is the only number such that adding one is the bitwise negation of
  // itself. Other than -1, of course, so we account for that

  // We also check if x = -1 too.
  int add1 = x + 1;
  return !(~x ^ add1) & ~!add1;
  /*
   *     ~x == x + 1 && x != -1
